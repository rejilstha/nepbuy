<?php
	...
	function login($username_email, $password, $connection) {
		$check_sql_string = "SELECT COUNT(*) as COUNT FROM nepbuy_users WHERE ( NAME='".$username_email."' OR EMAIL='".$username_email."' ) AND PASSWORD='".md5($password)."'";
		...
		if($count['COUNT'] == 0) {
			...
			$check_sql_string = "SELECT PK_USER_ID FROM nepbuy_users WHERE NAME='".$username_email."' OR EMAIL='".$username_email."'";
			$chk_st_id = oci_parse($connection, $check_sql_string);
			oci_execute($chk_st_id);
